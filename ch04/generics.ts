...
interface TLogIntegerface<T extends IPrint> {
    logToConsole(iPrintObj: T): void;
}

class LogClass<T extends IPrint>implements TLogIntegerface<T> {
    logToConsole(iPrintObj: T): void {
        iPrintObj.print();
    }
}
...
let classAInstace = createClassInstance(ClassA);

console.log(`${JSON.stringify(classAInstace)}`);